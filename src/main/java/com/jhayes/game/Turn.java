...
public final class Turn {
	...
	/** the id of the current main.java.com.jhayes.player; corresponds to the index of the main.java.com.jhayes.player in {@link ColdWar2100#players} */
	...
	private Turn() {
		...
		String temp = (String) JOptionPane.showInputDialog(null, "Choose Maximum number of turns:", "Max turns",
				JOptionPane.QUESTION_MESSAGE, null, maxTurnChoices, maxTurnChoices[3]);
		if (temp.equals("\u221E")) {
			temp = "0";
		}
		this.maxTurns = Integer.valueOf(temp);
		if (this.maxTurns == 0) {
			// main.java.com.jhayes.player chose infinite turns; this creates issues because of the recursive nature of doTurn(). Thus there upper limit is 1000 turns.
			this.maxTurns = 1000;
		}
	}
	 ...
	 *
	 * @return true when the main.java.com.jhayes.game is over, either by victory or maximum number of turns, otherwise false
	 */
		...
		for (Player p : ColdWar2100.getPlayers()) {
			...
			while (p.getOwnedCities().stream().anyMatch(ph -> ph.getMissilesBasedHere().size() != 0)) {
				choice = JOptionPane.showConfirmDialog(null, "Would you like to attack?", "Attack Phase",
						JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, VisualResources.iconII);
			...
			do {
				...
				if (choice == JOptionPane.NO_OPTION) {
					...
					new PopulationHub(p, "defacto", Integer.valueOf(temp[0]), Integer.valueOf(temp[1])).addToDS();
				}
			} while (true);
		// end the turn
		}
	...
	//turn stuff
		...
		for (Player p : ColdWar2100.getPlayers()) {
			...
			// launch attack(s)
			try {
				GEP.sendEvent(server.getConnection(p.getID()).getOut(), GEP.EnumEvents.ATTACKED);
			} catch (IOException e) {
				e.printStackTrace();
			}
			...
			// found new PopHubs
			try {
				GEP.sendEvent(server.getConnection(p.getID()).getOut(), GEP.EnumEvents.FOUNDED);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
	 ...
	 *
	 * @return the number of players still in the main.java.com.jhayes.game
	 */
